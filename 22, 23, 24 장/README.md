...
대답: View를 테스트해볼 수 있는 요소는 크게 세 가지로 나눌 수 있을 것 같아요.
(데이터와 관련된) 이벤트, 표시하는 데이터, 스타일.
이벤트와 표시하는 데이터는 GUI에서 수행하는 행위이므로 Presenter 단에서 쉽게 테스트 할 수 있겠죠?

스타일의 경우도 조건부 스타일을 걸었을 때 ー padding 값을 예로 들어 봅시다. ー
padding의 값이 제대로 바뀌었는지는 쉽게 테스트 할 수 있지만,
이게 다른 UI와 엉키지 않고 제대로 배치되었는가는 현재 프론트에서 거의 대부분 차용하는 반응형 페이지에선
코드 레벨에서 테스트하는 것은 매우 힘들겠죠.
```
...
대답: p.231의 일차원 경계의 한 예시로, 아키텍처의 양방향 경계를 확실히 긋는 용도의 인터페이스로 보입니다.
조금 더 확장된 그림은 p.215의 다이어그램 우측 하단에 있는데,
이 역시 {프레젠터 & 컨트롤러}, {유스케이스 인터렉터}를 완전히 격리시켜 놓은 모습이네요.

일단 필자가 이 그림에서 주목하라고 하는 점은 의존성의 방향이지만,
경계를 확실히 나누는 이 구조도 클린한 아키텍처의 한 부분이라고 볼 수 있겠네요.
...
대답: > 이 말을 `전사적으로 이 엔티티 레이어를 공유한다`라고 받아들여도 될까요?

네, 저는 그렇게 생각합니다.

> 만약 그렇게 된다면, 서로 다른 언어의 애플리케이션에서는 어떤 방식으로 공유되어야 할까요?

일단 저는 RPC(Remote Procedure Call)가 생각나네요.

"네트워크로 연결된 서버 상의 프로시저(함수, 메서드 등)를 원격으로 호출할 수 있는 기능입니다.
네트워크 통신을 위한 작업 하나하나 챙기기 귀찮으니.
통신이나 call 방식에 신경쓰지 않고 원격지의 자원을 내 것처럼 사용할 수 있죠.
IDL(Interface Definication Language) 기반으로 다양한 언어를 가진 환경에서도 쉽게 확장이 가능하며
인터페이스 협업에도 용이하다는 장점이 있습니다."
...
대답: MVP는 MVC의 파생 패턴입니다. MVC에서 View가 Model을 직접 참조하지 않고, Controller을 통해 참조하면 MVP 패턴입니다.

리액트의 많은 상태관리 라이브러리는 Flux 패턴을 사용합니다.
저는 View에서 Model로의 직접적인 접근을 막는 MVVM 패턴에, 사용가능한 로직을 미리 정해놓은 Facade 패턴을 섞은 패턴이라고 봅니다.
Flux 패턴의 특징은 단방향 데이터 흐름입니다.
데이터 조작 로직 호출은 View에서 Dispatcher로 흐르고, Dispatcher는Store(Model)에 접근하여 실제 데이터 조작을,
Store은 변경된 데이터를 View로 보내는 방식입니다.
```
...
대답: 테스트 용이성 또한 좋은 아키텍처의 필요조건입니다(p.225).
테스트를 쉽게 한다는것 그 자체에도 가치가 있습니다.

리액트에선 UI를 "컴포넌트(책의 앞장에서 언급됐던 컴포넌트보단 더 작은 단위)"로 구성합니다.
클래스와 함수형 컴포넌트가 있는데, 함수형 컴포넌트에선 return 값이 실제로 렌더링되는 View입니다.
데이터와 비즈니스 로직은 hook이라는 또다른 함수를 컴포넌트 안에서 호출해 사용할 수 있습니다.
로직들이 들어있어 테스트가 용이한 hook과, 테스트가 어려운 험블 객체인 View를 분리한 사례라고 볼 수 있을 것 같습니다.
